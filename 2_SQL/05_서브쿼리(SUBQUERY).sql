...
--다중열 서브쿼리 : 서브쿼리의 조회 결과값이 한행이지만 컬럼이 어러개일 때
--다중행 다중열 서브쿼리 : 서브쿼리의 조회 결과값이 여러행 여러컬럼일 때

>>서브쿼리의 결과값에 따라서 서브쿼리의 앞쪽에 연산자가 달라진다
*/
...
--4) 노옹철 사원보다 많이받는 사원들의 사번, 이름, 부서명, 급여조회
...
> ALL(서브쿼리) : 여러개의 모든 결과값 보다 클 경우
< ALL(서브쿼리) : 여러개의 모든 결과값 보다 작을 경우
*/
...
--박나라 사원과 같은 직급코드, 같은 사수를 가지고 있는 사번, 사원명, 직급코드, 사수번호
...
WHERE JOB_CODE = 'J2' AND SALARY = 3700000
    ...
    OR JOB_CODE = 'J4' AND SALARY = 2200000
