...
public class Book {
	 ...
	 * default, private
	 * 
	 * 필드와 메소드에 사용할 수 있는 접근 제한자
	 * public : 어디서든(같은 패키지, 다른 패키지 모두) 접근 가능
	 * protected : 같은 패키지 + 다른패키지일 경우 상속관계에서만 접근 가능
	...
	//생성자
	//객체를 생성할 때 데이터를 초기화 해주는 작업이 필수적이기 때문에
	//초기화를 위한 메스드를 만들어준다 -> 생성자라는 규칙이 생김
...
//	}
	 ...
	 * -개발자를 직접 생성자를 작성하면 기본생성자를 만들어주지 않는다
