...
/*
    useEffect
    컴포넌트가 렌더링 될때 특정 작업을 실행할 수 있도록 하는 react hook
...
const UseEffectTest = () => {
    ...
    //렌더링이 되었을 때, 그리고 state가 변경될 때 마다 console.log가 실행되었다.
    //componentDidMount & componentDidUpdate의 역할을 대체할 수 있다.
    useEffect(() => {
        console.log("의존성 배열이 없는 useEffect 호출")
    })

     //렌더링이 되었을 때 한번만 실행이 된다.(최초마운트)
     //componentDidMount의 역할을 대체하고 있다.
    useEffect(() => {
        console.log("빈배열을 의존성 배열로 가진 useEffect호출")
    }, [])

    //렌더링이 되었을 때 그리고 name state가 변경 되었을 때 호출된다.
    //의존성 배열 안에 있는 state가 변하면 해당 effect함수를 호출해 준다.
    //componentDidMount & componentDidUpdate의 역할을 대체할 수 있다.
    useEffect(() => {
