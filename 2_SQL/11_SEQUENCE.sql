...
[START WITH 시작숫자] -> 처음 발생시킬 시작값 지점(기본값 1)
[INCREMENT BY 숫자] -> 몇씩 증가시킬건지 (기본값 1)
[MAXVALUE 숫자] -> 최대값 지정(기본값 매우큼)
[MINVALUE 숫자] -> 최소값 지정(기본값 1)
[CYCLE | NOCYCLE] -> 값 순환여부(기본값 NOCYCLE)
[CACHE 바이트 크기| NOCACHE] -> 캐시메모리 할당(기본값 CACHE 20)

*캐시메모리 : 미리 발생할 값들을 생성해서 저장해두는 공간
            매번 호출할 때 마다 새로운 번호를 생성하는게 아니라
            캐리메모리 공간에 미리 생성된 값들을 가져다 쓸 수 있다.(속도가 빨라짐)
...
--> NEXTVAL를 한번도 수행하지 않은 이상 CURRVAL를 할 수 없다
...
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; --315로 최대값 310을 넘겼다
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
        VALUES(SEQ_EID.NEXTVAL, '김사원', '111111-3111111', 'J6', SYSDATE);