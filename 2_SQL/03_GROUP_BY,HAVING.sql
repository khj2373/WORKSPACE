...
--각 직급별 총 사원수, 보너스를 받는 사람수, 급여 합, 평균급여, 최저급여, 최고급여 (정렬 = 직급 오름차순)
SELECT JOB_CODE 직급, COUNT(*) 사원수, COUNT(BONUS) "보너스 받는 사람", SUM(SALARY) 급여합, ROUND(AVG(SALARY)) 급여평균, MIN(SALARY) 최저급여, MAX(SALARY) 최고급여
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
/*
[HAVING 절]
그룹에 대한 조건을 제시할 때 사용되는 구문(주로 그룹 함수식을 가지고 조건을 만든다)
*/
...
--각 부서별 평근급여가 300만원 이상인 부서들만 조회(부서코드, 평균급여)
SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
WHERE SALARY >= 3000000 --잘못된 방법
GROUP BY DEPT_CODE;
...
/*
SELECT * || 조회하고싶은 컬럼 || 함수식 || 산술연산 --5
FROM TABLE --1
WHERE 조건식 --2
GROUP BY 그룹의 기준이 되는 컬럼 --3
HAVING 조건식 --4
...
UNION ALL | 합집합 + 교집합(중복 상관하지 않고 전부 더한 결과값)
