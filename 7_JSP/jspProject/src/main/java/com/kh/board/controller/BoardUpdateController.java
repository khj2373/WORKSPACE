...
public class BoardUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(JakartaServletFileUpload.isMultipartContent(request)) {
			...
			//전달된 파일을 저장시킬 폴더경로 가져오기
			String savePath = request.getServletContext().getRealPath("/resources/board_upfile/");
			
			//DiskFileItemFactory(파일을 임시로 저장) 객체 생성
			DiskFileItemFactory factory = DiskFileItemFactory.builder().get();
			
			//JakartaServletFileUpload http요청으로 들어온 파일데이터를 파싱 -> 개별 FileItem 객체로 변환
			...
			//새 첨부파일 x					b, null -> board update
			//새 첨부파일 o, 기존 첨부파일 o	b, fileNo -> board update, attachment update
			//새 첨부파일 o, 기존 첨부파일 x	b, refBoardNo -> board update, attachment insert
