...
public class MemberController {
	...
	public Member loginMember(String id, String pwd) {
		...
		if(m == null) {
			new MemberMenu().displayFail("로그인 실패, ID와 PASSWORD를 다시 확인해주세요.");
			return null;
		} else {
			new MemberMenu().displaySuccess(id + "님 환영합니다.");
			return m;
		}
	}
	...
	public void searchMemberByName(String keyword) {
		List<Member> list = new MemberService().searchMemberByName(keyword);
		if(list.isEmpty()) { //list가 비어있을 경우
			new MemberMenu().displayNoData(keyword + "으로 조회된 결과가 없습니다");
		} else { //list에 데이터가 있을 경우
			new MemberMenu().displayMemberList(list);
		}
	}
