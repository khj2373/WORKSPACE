...
EX)
회원탈퇴시 기존 회원 테이블에 데이터 DELETE한 후 곧바로 탈퇴한 회원들만 따로 보관하는 테이블에
자동으로 INSERT 시켜야한다
신고횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트로 처리하게끔
입출고에 대한 기록(INSERT)될 때마다 해당 상품에 대한 재고 수량을 매번 수정(UPDATE)해야한다
...
트리거 생성 구문
[표현식]
CREATE [OR REPLACE] TRIGGER 트리거면
...
VALUES(SEQ_EID.NEXTVAL, '최현준', '123123-1231234', 'D7', 'J7', SYSDATE);
...
--상품번호 중복 안되게끔 매번 새로운 번호를 발생시키는 시퀀스 생성
...
CREATE TABLE TB_PRODETAIL(
    DECODED NUMBER PRIMARY KEY, --이력번호
...
--300번 상품이 오늘 날짜로 10개 입고
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECODE.NEXTVAL, 300, SYSDATE, 10, '입고');
...
--310번 상품이 오늘 날짜로 출고
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECODE.NEXTVAL, 310, SYSDATE, 7, '출고');

--300번 상품이 오늘 날짜로 100개 입고
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DECODE.NEXTVAL, 300, SYSDATE, 100, '입고');