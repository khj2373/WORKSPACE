...
FROM 조회할 테이블
WHERE 조건식
ORDER BY 정렬 기준이 될 컬럼 || 별칭 || 컬럼순번 [ASC || DESC] [NULLS FIRST || NULLS LAST]

-ASC : 오름차순(작은 값으로 시작해서 값이 점점 커지는 것) -> 기본값
-DESC : 내림차순(큰 값으로 시작해서 값이 점점 줄어드는 것)

-NULL은 기본적으로 가장 큰 값으로 분류해서 정렬한다.
-NULLS FIRST : 정렬하고자 하는 컬럼 값에 NULL이 있을 경우 해당 데이터 맨 앞에 배치(DESC일때 기본값)
-NULLS LAST : 정렬하고자 하는 컬럼 값에 NULL이 있을 경우 해당 데이터 맨 마지막에 배치(ASC일때 기본값)
*/

SELECT *
FROM EMPLOYEE
--ORDER BY BONUS; --기본 값 오름차순
--ORDER BY BONUS ASC;
...
--ORDER BY 2 DESC;
ORDER BY 연봉 DESC; --순번 사용 가능 오라클은 전부 1부터 시작

/*
<함수 FUNCTION>
전달한 컬럼값을 받아서 함수를 실행한 결과를 반환
...
INSTR(컬럼 || '문자열', '찾고자하는 문자', ['찾을 취치의 시작값, 순번]) -? 결과는 NUMBER
*/
SELECT INSTR('ABCDDDES', 'B') FROM DUAL; --앞쪽에 있는 첫 B는 2번째 위치해 있다.
--찾을 위치 시작값 : 1, 순번 1 => 기본값
SELECT INSTR('ABCDDDES', 'D', -1) FROM DUAL; --뒤에서부터 찾고 위치를 읽을 때는 앞에서부터 읽어준다.
SELECT INSTR('ABCDDDES', 'D', 1, 2) FROM DUAL; --순번을 제시하려면 위치의 시작값을 표시해야 한다.
SELECT INSTR('ABCDDDES', 'D', 1, 3) FROM DUAL;
...
SELECT EMP_NAME, EMP_NO, SUBSTR(EMAIL, 1, INSTR(EMAIL, '@') - 1)
FROM EMPLOYEE;
...
[표현법]
LPAD || RPAD(STR, 최종적으로 반환할 문자열의 길이, [덧붙이고자 하는 문자])

문자열에 덧붙이고자 하는 문자를 왼쪽 또는 오른쪽에 붙여서 최종길이 N만큼 문자열을 반환
*/
...
--사원들의 사원명, 주민번호 조회("701011-1XXXXXX")
...
LTRIM || RTRIM(STR, [제거하고자 하는 문자들])

문자열의 왼쪽 혹은 오른쪽에서 제거하고자 하는 문자들을 찾아서 제거한 나머지 문자열 반환
*/
SELECT LTRIM('     K   H     ') FROM DUAL;
SELECT LTRIM('ACBABCAABKKH', 'ABC') FROM DUAL;
SELECT LTRIM('5131241245ACBABCAABKKH', '0123456789') FROM DUAL;
...
SELECT TRIM('Z' FROM 'ZZZZKHZZZZZZZZZ') FROM DUAL; --양쪽에 있는 공백 제거

SELECT TRIM(LEADING 'Z' FROM 'ZZZZKHZZZZZZZZZ') FROM DUAL;
SELECT TRIM(TRAILING 'Z' FROM 'ZZZZKHZZZZZZZZZ') FROM DUAL;
...
/*
REPLACE
특정문자열에서 특정 부분을 다른 부분으로 교체
REPLACE(문자열, 찾을 문자열, 변경 문자열)
*/
...
--ADD_MONTHS : 특정 날짜에 NUM개월수를 더해서 반환
SELECT ADD_MONTHS(SYSDATE, 4) FROM DUAL;

--근로자 테이블에서 사원명, 입사일, 입사 후 3개월의 날짜 조회
...
/*
[형변환 함수]
TO_CHAR : 숫자타입 또는 날짜타입의 값을 문자타입으로 변환시켜 주는 함수
...
SELECT TO_CHAR(12, '99999') FROM DUAL; --9의 자리수만큼 공간확보, 오른쪽 정렬
SELECT TO_CHAR(12, '00000') FROM DUAL; --0의 자리수만큼 공간확보, 빈칸을 0으로 채움
SELECT TO_CHAR(2000000, 'L9999999') FROM DUAL; --현재 설정된 나라의 화폐 단위로 나타남
...
SELECT TO_CHAR(SYSDATE, 'YYYY'),
       TO_CHAR(SYSDATE, 'YY'),
       TO_CHAR(SYSDATE, 'RRRR'), --RR룰이 따로 존재 -> 50SUS DLTKD RKQTDL + 100 EX) 1954
...
SELECT TO_DATE(240807) FROM DUAL; --50년 미만은 자동으로 20XX으로 설정 50년 이상은 19XX으로 설정

SELECT TO_DATE('020505') FROM DUAL; --숫자는 0으로 시작할 수 없다

SELECT TO_DATE('020505 120500', 'YYMMDD HH24MISS') FROM DUAL; --포맷을 정해줘야 시, 분, 초를 표시할 수 있다
...
--부서 코드가 D5인 사원들의 총 연봉
SELECT SUM(SALARY * 12)
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';

--2. AVG(NUM) : 해당 컬럼값들의 평균을 구해서 반환
