...
<body>
	...
	*변수 선언과 동시에 초기화(c:set var="변수명" value="값" [scope="저장객체"])
	...
	-해당 변수를 어떤 scope에 setAttribute를 통해 담아둘지 결졍할 수 있다.
	...
	*변수 삭제(c:remove var="제거하고 싶은 변수명" [scope=""])
	...
	-scope 지정 생략시 모든 scope에서 해당 변수를 다 찾아서 제갛낟.
	=> 즉, 해당 scope에서 .removeAttribute를 통해 제거하는 거라고 생각하면 된다.
	</pre>
	...
	*변수(데이터) 출력 (c:out value="출력하고자 하는 값" [default="기본값"] [escapeXml="true | false"])
	...
	-조건식은 test 속성에 작성(단, EL구문으로 기술해야 한다.)
	</pre>
	...
	향상된 for문 - (c:forEach var="변수명" items="순차적으로 접근하고자 하는 객체(배열/컬렉션)" [varStatus="현재접근한 요소의 상태값"])
	</pre>
	...
	-구분자를 통해서 분리딘 각각의 문자열에 순차적으로 접근하여 반복수행
	-JAVA의 문자열.split("구분자")과 비슷("김,현,준".split(",") -> ["김","현","준"])
	</pre>
