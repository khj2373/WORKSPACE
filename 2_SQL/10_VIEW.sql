...
/*
뷰 컬럼에 별칭 부여
서브쿼리의 SELECT절에 함수식이나 산술연산식이 기술되어 있다면 반드시 별칭을 부여해야 한다
*/
...
SELECT *
FROM VW_JOB; --논리테이블(실제 데이터가 담겨있지 않다)
...
------------------------------------------------------------------------------
/*
DML 명령어로 조작이 불가능 한 경우가 많다

1) 뷰에 정의되어 있지 않은 컬럼으로 조작하려고 하는 경우
...
대부분 뷰는 조회를 목적으로 생성한다. 뷰를 통한 DML 조작은 안하는게 좋다.
*/
...
1) OR REPLACE : 기존에 동일한 뷰가 있을 경우 갱신하고, 없을 경우 새로 생성
2) FORCE || NOFORCE
    >>FORCE : 서브쿼리에 기술된 테이블이 존재하지 않아도 뷰를 생성
    >>NOFORCE : 서브쿼리에 기술된 테이블이 존재하는 테이블이어야만 한다(기본값)
3) WITH CHECK OPTION : DML시 서브쿼리에 기술된 조건에 부합한 값으로만 DML이 가능
