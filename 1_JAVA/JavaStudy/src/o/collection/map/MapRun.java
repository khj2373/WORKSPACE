...
public class MapRun {
	...
	public static void main(String[] args) {
		...
		//계층 구조를 보면
		//List계열, set계열 클래스들은 Collection을 구현한 클래스
		//=>객체를 추가하고자 할때 공통적으로 add메소드를 이용
		
		//Map계열은 Collection을 구현한 클래스가 아니다
		//=> 객체를 추가하고자 할때 put메소드를 이용(key + value한쌍으로 담아야 한다.)
		...
		//5. remove(Object key) => 해당 키값을 찾아서 그 밸류 세트를 삭제시켜주는 메소드
		hm.remove("다이제");
		System.out.println(hm);
		...
		while(it.hasNext()) {
			Entry entry = (Entry)it.next(); //Entry객체 안에 key, value값이 다 담겨져 있다
