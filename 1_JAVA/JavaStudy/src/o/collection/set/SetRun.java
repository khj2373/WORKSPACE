...
public class SetRun {
	...
	public static void main(String[] args) {
		...
		System.out.println(hsi); //저장순서 유지x 중복된 데이터(동일객체) 보관 불가
		...
		hsi2.add(new Student("김현준", 22, 95));
		hsi2.add(new Student("홍길동", 42, 75));
		hsi2.add(new Student("사람1", 41, 60));
		hsi2.add(new Student("김현준", 22, 95));
		
		//동일객체 : 각 객체마다 hashCode 결과가 일치하고, equals 메소드로 비교시 true일때
		
		//Student에 equals() 오버라이딩 => "실제 각 필드에 담긴 데이터"들이 다 일치하면 true/일치하지 않으면 false
		...
		while(it.hasNext()) {//다음에 가져올 값이 있는가
			Object obj = it.next();
